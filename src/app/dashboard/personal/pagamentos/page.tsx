...
const STATUS_LABELS: Record<string, { label: string; color: string }> = {
  paga: { label: "Pago — aguardando confirmação", color: "text-blue-400 bg-blue-500/10 border-blue-500/30" },
  confirmada: { label: "Pagamento liberado", color: "text-green-400 bg-green-500/10 border-green-500/30" },
...
export default function PagamentosPersonalPage() {
  ...
  const liberados = aulas.filter((a) => a.status === "confirmada");
  const totalRecebido = pagos.reduce((s, a) => s + a.valor, 0);
  const totalLiberado = liberados.reduce((s, a) => s + a.valor, 0);
  ...
  return (
    <main className="max-w-3xl mx-auto px-6 py-10">
      ...
      <p className="text-zinc-500 text-sm mb-6">Todos os recebimentos pelas aulas na plataforma.</p>
      ...
      {aulas.length > 0 && (
        <div className="grid grid-cols-3 gap-4 mb-8">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4">
            <p className="text-xs text-zinc-500 mb-1">Liberado</p>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4">
            ...
            <p className="text-xl font-black text-blue-400">
              R$ {(totalRecebido - totalLiberado).toFixed(2).replace(".", ",")}
            </p>
          </div>
        </div>
      )}
      ...
      {aulas.length === 0 ? (
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-12 flex flex-col items-center gap-4 text-center">
          ...
          <p className="text-zinc-400 font-medium">Nenhum pagamento recebido ainda.</p>
          <p className="text-zinc-600 text-sm max-w-xs">
            Quando um aluno contratar e pagar uma aula, o pagamento aparecerá aqui.
          </p>
        </div>
      ) : (
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-white">
                      Aula — {aula.aluno.nome} {aula.aluno.sobrenome}
                    </p>
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-lg font-black text-yellow-400 flex items-center gap-1 justify-end">
