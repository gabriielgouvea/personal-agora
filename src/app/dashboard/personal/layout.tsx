...
import { User, Calendar, CreditCard, LogOut, ChevronDown, Settings, CalendarDays, DollarSign, Star, Wallet } from "lucide-react";
...
const NAV_ITEMS = [
  ...
  { label: "Financeiro", href: "/dashboard/personal/financeiro", icon: Wallet },
...
export default function DashboardPersonalLayout({ children }: { children: React.ReactNode }) {
      ...
      <header className="sticky top-0 z-30 bg-black/80 backdrop-blur border-b border-zinc-800">
        <div className="max-w-6xl mx-auto flex items-center justify-between h-16 px-6">
              ...
              {NAV_ITEMS.map((item) => (
                <Link
                  ...
                  className={`px-3 py-2 rounded-lg text-sm font-medium transition flex items-center gap-2 ${
                    pathname === item.href ? "bg-yellow-500/10 text-yellow-500" : "text-zinc-400 hover:text-white"
                  }`}
                >
          ...
          <div className="relative" ref={menuRef}>
            ...
            {menuOpen && (
              <div className="absolute right-0 mt-2 w-52 bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl overflow-hidden">
                ...
                {NAV_ITEMS.map((item) => (
                  <Link
                    ...
                    className={`flex items-center gap-2 px-4 py-2.5 text-sm transition ${
                      pathname === item.href ? "text-yellow-500 bg-yellow-500/5" : "text-zinc-300 hover:bg-zinc-800"
                    }`}
                  >
                    <item.icon className="w-4 h-4" />
                    {item.label}
